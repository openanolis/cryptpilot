...
CryptPilot uses Remote Attestation to perform measurement of the root file system. The expected value of the root file system is recorded in the initrd image, while the measurement of the initrd itself is stored in a non-rewritable Event Log (CCEL). Together with the kernel's dm-verity mechanism, this ensures the integrity protection of the root file system.
