...
转换后的磁盘采用LVM管理存储布局，在名为system的卷组中创建三个逻辑卷：rootfs逻辑卷存储收缩后的rootfs数据，rootfs_hash逻辑卷存储dm-verity哈希树，data逻辑卷作为差异层存储空间。当使用overlayfs机制时，data卷挂载为文件系统存储可写层；当使用dm-snapshot机制时，data卷作为块设备直接作为COW（Copy-On-Write）存储。
...
data卷的初始化也在此一阶段完成。服务会先检查data逻辑卷是否存在，若不存在则创建并占用卷组全部剩余空间。若data卷已存在，则将其扩容至卷组剩余空间。

**data卷解密流程**：

```text
data_lv → dm-crypt → dm-integrity(可选) → 解密后的data卷块设备
```

在data逻辑卷创建或扩容后，服务获取data卷的passphrase，判断是否需要重新初始化，格式化LUKS2卷，得到解密后的data卷块设备。解密后的data卷块设备将用于承载差异层数据。系统支持两种差异层实现机制，通过`rw_overlay_backend`配置选择，默认为dm-snapshot。

#### 3.1.2 overlayfs数据卷准备（当使用overlayfs机制时）

当配置`rw_overlay_backend = "overlayfs"`时，服务会在解密后的data卷块设备上创建ext4文件系统，用于存储overlay upperdir和workdir。
...
当配置`rw_overlay_backend = "dm-snapshot"`（默认）时，服务会将解密后的data卷块设备直接作为COW存储使用。随后继续构建dm-snapshot设备链，使dracut能够直接挂载可写的根文件系统。
...
rootfs_lv → [dm-crypt] → dm-verity ──┬─→ dm-linear ──→ dm-snapshot ──→ /sysroot
                                  ...
                                  dm-zero                   │
                                     │                   COW设备
                              (大小等于COW设备)    (解密后的data卷 或 zram)
```

构建过程分为以下步骤：

1. **准备COW设备**：根据`rw_overlay_location`配置准备写时复制存储
   - `rw_overlay_location = "ram"`：创建zram内存块设备，大小等于系统内存
   - `rw_overlay_location = "disk"`或`"disk-persist"`：使用data逻辑卷作为COW设备
...
#### 3.3.1 overlayfs数据卷启用（当使用overlayfs机制时）

当配置`rw_overlay_backend = "overlayfs"`时，使用文件级覆盖机制。服务将原始dm-verity设备挂载的目录作为overlayfs的lowerdir。
...
#### 3.3.2 dm-snapshot数据卷启用（当使用dm-snapshot机制时）

当配置`rw_overlay_backend = "dm-snapshot"`（默认）时，dm-snapshot设备链已在`cryptpilot-fde-before-sysroot`阶段准备完成。dracut直接挂载dm-snapshot设备到`/sysroot`，该挂载即为可写状态。
