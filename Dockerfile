...
# Release stage for cryptpilot-crypt
FROM alibaba-cloud-linux-3-registry.cn-hangzhou.cr.aliyuncs.com/alinux3/alinux3:latest AS release-crypt
...
# Copy RPM from builder
COPY --from=builder /root/rpmbuild/RPMS/x86_64/cryptpilot-crypt-*.rpm /tmp/

# Install Crypt package and its dependencies
RUN yum install -y /tmp/cryptpilot-crypt-*.rpm && \
    yum clean all && \
    rm -rf /var/cache/yum /tmp/*.rpm

CMD ["cryptpilot-crypt"]


# Release stage for cryptpilot-verity
FROM alibaba-cloud-linux-3-registry.cn-hangzhou.cr.aliyuncs.com/alinux3/alinux3:latest AS release-verity
...
# Copy RPM from builder
COPY --from=builder /root/rpmbuild/RPMS/x86_64/cryptpilot-verity-*.rpm /tmp/

# Install Verity package and its dependencies
RUN yum install -y /tmp/cryptpilot-verity-*.rpm && \
    yum clean all && \
    rm -rf /var/cache/yum /tmp/*.rpm

CMD ["cryptpilot-verity"]
